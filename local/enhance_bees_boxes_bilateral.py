#!/usr/bin/env python3
"""
enhance_bees_boxes_bilateral.py
================================
Same pipeline as enhance_bees_boxes.py but with a bilateral filter at
step 5 instead of the SNN filter.

Bilateral filter: edge-preserving smoothing via joint spatial × range
Gaussian weighting — handles intensity gradients smoothly and is
available natively in OpenCV (cv2.bilateralFilter).

Outputs (pipeline_output/bee_boxes_bilateral/):
...
OUT_ROOT = "pipeline_output/bee_boxes_bilateral"
...
def enhance(crop_gray, out_h, out_w):
    ...
    # d=9: neighbourhood diameter; sigmaColor=55: range sigma (intensity);
    # sigmaSpace=9: spatial sigma.  Applied twice for stronger smoothing
    # while keeping edge sharpness.
    work = cv2.bilateralFilter(work, d=9, sigmaColor=55, sigmaSpace=9)
    work = cv2.bilateralFilter(work, d=9, sigmaColor=55, sigmaSpace=9)
